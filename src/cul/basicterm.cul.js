...
export const mort_rate_select_index = () =>
  Math.max(0, Math.min(5, duration())); // used to lookup mort_rate table

// I can hoist this change to playground.cul.js, but it provides context for pols_if_at behaviour here
export const zero_decrement_experience = () =>
...
export const pols_if_at = () => {
  ...
  if (timing() == "BEF_MAT") {
    if (t() == 0 && pols_if_init() && 0 /* DN: disabling */)
      return pols_if_init();
    return (
      pols_if_at({ t_in: t() - 1, timing_in: "BEF_DECR" }) -
      pols_lapse({ t_in: t() - 1 }) -
      pols_death({ t_in: t() - 1 })
    );
  }
  ...
  if (timing() == "BEF_DECR")
    return pols_if_at({ timing_in: "BEF_NB" }) + pols_new_biz();
  return console.error("bad timing_in !");
};

export const pols_lapse = () =>
  is_active()
    ? (pols_if_at({ timing_in: "BEF_DECR" }) - pols_death()) * lapse_rate_mth()
    : 0;
...
export const pols_new_biz = () => {
  if (duration_mth() == 0) return policy_count();
  else return 0;
};

export const pols_death = () =>
  is_active() ? pols_if_at({ timing_in: "BEF_DECR" }) * mort_rate_mth() : 0;

// monthly
export const premium_pp = () =>
  Math.round((sum_assured() / 1000) * premium_rate_per_mille() * 100) / 100; // round 2 decimal places
...
export const premiums = () =>
  premium_pp() * pols_if_at({ timing_in: "BEF_DECR" });
...
export const proj_len = () =>
  Math.max(12 * policy_term() - duration_mth({ t_in: 0 }) + 1, 0);

// todo think re actuals (new)
export const is_active = () =>
  !(duration_mth() < 0 || duration_mth() > policy_term() * 12);
...
export const net_premium_pp = () => {
  return (
    -pv_fut_claims({ t_in: 0, duration_mth_0_in: 0 }) /
    pv_fut_pols_if({ t_in: 0, duration_mth_0_in: 0 })
  );
};

export const premium_rate_per_mille = () => premium_rate() * 1000; // uses table from basicterm-tables.cul.js
