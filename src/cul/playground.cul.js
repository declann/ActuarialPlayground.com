...
export const lapse_rate = () => {
  if (t() < stress_delay())
    return original_lapse_rates()
      ? Math.max(0.1 - 0.02 * duration(), 0.02)
      : lapse_rate_();
  else
    return (
      lapse_rate_factor() *
      (original_lapse_rates()
        ? Math.max(0.1 - 0.02 * duration(), 0.02)
        : lapse_rate_())
    );
};
...
export const mort_rate_Y1_add_per_mille = () =>
  mort_rate_Y1_add_per_mille_in ?? 0;

export const mort_rate_addition = () => {
  if (t() < 12 + stress_delay()) return mort_rate_Y1_add_per_mille() / 1000;
  else return 0;
};
...
export const update_pricing_lapse_rates = () =>
...
export const net_premium_pp = () => {
  if (pricing_projection())
    return net_premium_pp_({
      ...
      original_lapse_rates_in: !update_pricing_lapse_rates(),
      discounting_on_in: true,
      sex_in: gender_neutral_pricing() ? "F" : sex(),
      timing_in: "BEF_DECR",
...
export const premium_due = () => {
  if (premium_payment_frequency() == "single") return t() == -duration_mth_0();
...
export const mort_rate_recalc = () =>
  Math.min(
    1,
    Math.min(1, a() * Math.exp(b() * Math.pow(age(), c()))) *
      Math.pow(1.1, mort_rate_select_index()) *
      (sex() == "M" ? 1.2 : 1),
  );
...
export const status = () =>
  duration_mth_0() == 0 ? "New Business" : "In Force";
